package com.pjem.cidade.apicidade.cidades.service;
...
public class DistanceService {
    ...
    public DistanceService(final CidadesRepository cidadesRepository) {
        this.cidadesRepository = cidadesRepository;
    }

    public Double distanceByCubeInMeters(Long city1, Long city2) {
        log.info("distanceByCubeInMeters({}, {})", city1, city2);
        final List<Cidades> cities = cidadesRepository.findAllById((Arrays.asList(city1, city2)));

        Point p1 = cities.get(0).getLocation();
        Point p2 = cities.get(1).getLocation();

        return cidadesRepository.distancieByCube(p1.getX(), p1.getY(), p2.getX(), p2.getY());
    }
    ...
    public Double distanceUsingMath(final Long city1, final Long city2, final EarthRadius unit) {
        ...
        final Double[] location1 = StringLocationsUtils.transform(cities.get(0).getGeolocation());
        final Double[] location2 = StringLocationsUtils.transform(cities.get(1).getGeolocation());
